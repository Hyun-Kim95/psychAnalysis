...
## 운영 원칙
- 기본 전제: 이 레포는 **Gate 1 이후 운영/개선 단계**를 기본으로 본다. 고객사 **신규·처음부터 납품** 요청이 명시될 때만 전체 납품형 Gate 흐름을 기본 적용한다.
...
## 기본 진입 규칙
- 고객사 **전체 납품(신규·처음부터 진행)** 대화는 사용자가 스킬 이름을 말하지 않아도 `.cursor/rules/70-client-lifecycle-default.mdc`에 따라 `client-project-lifecycle`을 따른다(PRD·디자인 등 HUMAN 구간에서 멈춤). 구현 이후 **다축 검증·리뷰어 GATE**는 해당 스킬 **단계 4B~4D(선택)** 및 `docs/qa/reviewer-gate-rubric.md`를 참고한다.
