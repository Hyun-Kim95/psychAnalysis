-- V29+V30+V31 통합: 선택지 '거의 항상' + BAI 공식 21문항·선택지 (V28 이후 미적용 시 한 번에 반영)

-- [V29] 선택지 문구 '거의 매일' → '거의 항상'
UPDATE choice SET label = '거의 항상' WHERE label = '거의 매일';

-- [BAI] 선택지 문구 공식 4점으로 통일
...
-- BAI 불안검사 (상세): 19~21문항 선택지 없으면 추가
INSERT INTO choice (item_id, label, value, sort_order)
SELECT i.id, c.label, c.value, c.ord
FROM item i
JOIN assessment a ON a.id = i.assessment_id
