-- DB에 직접 실행: 선택지 '거의 항상' + BAI 공식 21문항·선택지 반영
-- 사용법: psql -U psychuser -d psychdb -f scripts/apply_bai_and_choice_manual.sql
-- 또는 pgAdmin/DBeaver 등에서 이 파일 전체 실행
...
SELECT
  (SELECT COALESCE(MAX(installed_rank),0) FROM flyway_schema_history) + row_number() OVER (),
  v.version, v.description, 'SQL', v.script, 0, current_user, NOW(), 0, true
FROM (VALUES
  ('29', 'choice label almost always', 'V29__choice_label_almost_always.sql'),
  ('30', 'bai official 21 items', 'V30__bai_official_21_items.sql'),
  ('31', 'bai official 21 items repair', 'V31__bai_official_21_items_repair.sql'),
  ('32', 'choice and bai official', 'V32__choice_and_bai_official.sql')
